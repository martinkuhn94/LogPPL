        ...
        for write_variable in transition.properties["writeVariable"]:
            ...
            if eq_op == None or len(eq_op) == 0:
                if dpn.get_variable_type(write_variable) in ["java.lang.Integer","java.lang.Long"]:
                    write_variables.append(_get_var_writing_function(transition.name,write_variable,WRITE_VARIABLES_GEN_VALUE_INTEGER_CODE,{"write_variable":write_variable},dpn))
                elif dpn.get_variable_type(write_variable) == "java.lang.Double":
                    write_variables.append(_get_var_writing_function(transition.name, write_variable,
                                                                     WRITE_VARIABLES_GEN_VALUE_DOUBLE_CODE,{"write_variable":write_variable},dpn))
                elif dpn.get_variable_type(write_variable) == "java.lang.String":
                    write_variables.append(_get_var_writing_function(transition.name, write_variable,
                                                                     WRITE_VARIABLES_GEN_VALUE_STRING_CODE,{"write_variable":write_variable,"samples":dpn.get_sample(write_variable)},dpn))
...
def _get_var_writing_function(transitionname,write_variable,default_distribution,parameters,dpn):
    if transitionname in dpn.get_distributions() and write_variable in dpn.get_distributions()[transitionname]:
        return dpn.get_distributions()[transitionname][write_variable].format(write_variable=write_variable)
    else:
        return default_distribution.format(write_variable=write_variable)
